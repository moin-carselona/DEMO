...
import { allcampaignsdata } from './API';
import { columns } from './Columns';
import { customStyles } from './CustomCss';
import DialogBox from './DialogBox/DialogBox';
import "./styles.css"
const AllCampaignsMain = () => {
  LocalBaseURL()
...
//  to fetch data on first time rendering 
  ...
  useEffect(() => {
    ...
    async function InvokedRendered() {
      const { data } = await allcampaignsdata(localKeyCheck)
    ...
    <div>
      {
        isLoading ?
          ...
          <DataTable
            ...
            actions={
              <>
              <button className="btn btn-sm btn-primary" onClick={(row) => PopUpallCampaignsBTN()}><i className="fa fa-plus-circle mr-2"></i>Add Service Price</button>
 ...
 {PopUpPostFormOpen && <DialogBox PopUpallCampaignsBTN={PopUpallCampaignsBTN} PopUpPostFormOpen={PopUpPostFormOpen} />}
      
    </div>
  )
}
export default AllCampaignsMain
