...
import { GetAllPointsTableData } from './API';
import { columns } from './Columns';
import { customStyles } from './CustomCss';
import DialogBox from './DialogBox/DialogBox';

const PointTableListMain = () => {
  LocalBaseURL()
...
//  to fetch data on first time rendering 
  ...
  useEffect(() => {
    ...
    async function InvokedRendered() {
      const { data } = await GetAllPointsTableData(localKeyCheck)
    ...
    <div>
      {
        isLoading ?
          ...
          <DataTable
            ...
            actions={
              <>
              <button className="btn btn-sm btn-primary" onClick={(row) => PopUpPointTableBTN()}><i className="fa fa-plus-circle mr-2"></i>Add Service Price</button>
 ...
 {PopUpPostFormOpen && <DialogBox PopUpPointTableBTN={PopUpPointTableBTN} PopUpPostFormOpen={PopUpPostFormOpen} />}
      
    </div>
  )
}
export default PointTableListMain
