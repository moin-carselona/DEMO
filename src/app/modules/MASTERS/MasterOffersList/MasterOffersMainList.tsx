...
import { GetAllMasterAPIData } from './API';
...
//  to fetch data on first time rendering 
  ...
  useEffect(() => {
    ...
    async function InvokedRendered() {
      const { data } = await GetAllMasterAPIData(localKeyCheck)
    ...
    <div>
      {
        isLoading ?
          ...
          <DataTable
            ...
            actions={
              <>
              <button className="btn btn-sm btn-primary" onClick={(row) => PopUpMasterBTN()}><i className="fa fa-plus-circle mr-2"></i>Add Master Offers</button>
 ...
 {PopUpPostFormOpen && <DialogBox PopUpMasterBTN={PopUpMasterBTN} PopUpPostFormOpen={PopUpPostFormOpen} />}
      
    </div>
  )
}
export default MasterOffersMainList
