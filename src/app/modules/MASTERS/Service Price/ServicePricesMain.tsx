...
import { GetallSercicePriceData } from './API';
import { columns } from './Columns';
import { customStyles } from './CustomCss';
import DialogBox from './DialogBox/DialogBox';

const ServicePricesMain = () => {
  LocalBaseURL()
...
//  to fetch data on first time rendering 
  ...
  useEffect(() => {
    ...
    async function InvokedRendered() {
      const { data } = await GetallSercicePriceData(localKeyCheck)
    ...
    <div>
      {
        isLoading ?
          ...
          <DataTable
            ...
            actions={
              <>
              <button className="btn btn-sm btn-primary" onClick={(row) => handleAddServicePrices()}><i className="fa fa-plus-circle mr-2"></i>Add New Service Price</button>
 ...
 {PopUpPostFormOpen && <DialogBox PopUpServicePrices={handleAddServicePrices} PopUpPostFormOpen={PopUpPostFormOpen} />}
      
    </div>
  )
}
export default ServicePricesMain
