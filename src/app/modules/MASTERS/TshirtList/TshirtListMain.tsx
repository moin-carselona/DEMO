...
import { getalltshirtdata } from './API';
import { columns } from './Columns';
import { customStyles } from './CustomCss';
import DialogBox from './DialogBox/DialogBox';

const TshirtListMain = () => {
  LocalBaseURL()
...
//  to fetch data on first time rendering 
  ...
  useEffect(() => {
    ...
    async function InvokedRendered() {
      const { data } = await getalltshirtdata(localKeyCheck)
    ...
    <div>
      {
        isLoading ?
          ...
          <DataTable
            ...
            actions={
              <>
              <button className="btn btn-sm btn-primary" onClick={(row) => PopUpTshirtBTN()}><i className="fa fa-plus-circle mr-2"></i>Add Service Price</button>
 ...
 {PopUpPostFormOpen && <DialogBox PopUpTshirtBTN={PopUpTshirtBTN} PopUpPostFormOpen={PopUpPostFormOpen} />}
      
    </div>
  )
}
export default TshirtListMain
