...
import { GetmasterTicketCategory } from './API';
import { columns } from './Columns';
import { customStyles } from './CustomCss';
import DialogBox from './DialogBox/DialogBox';

const MasterTicketCategoryListMain = () => {
  LocalBaseURL()
...
// to open add service form  
  const PopUpMasterTicketCategoryBTN = () => {
...
//  to fetch data on first time rendering 
  ...
  useEffect(() => {
    ...
    async function InvokedRendered() {
      const { data } = await GetmasterTicketCategory(localKeyCheck)
    ...
    <div>
      {
        isLoading ?
          ...
          <DataTable
            ...
            actions={
              <>
              <button className="btn btn-sm btn-primary" onClick={(row) => PopUpMasterTicketCategoryBTN()}><i className="fa fa-plus-circle mr-2"></i>Add Service Price</button>
 ...
 {PopUpPostFormOpen && <DialogBox PopUpMasterTicketCategoryBTN={PopUpMasterTicketCategoryBTN} PopUpPostFormOpen={PopUpPostFormOpen} />}
      
    </div>
  )
}
export default MasterTicketCategoryListMain
