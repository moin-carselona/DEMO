...
import { GetallPackagesData } from './API';
import { columns } from './Columns';
import { customStyles } from './CustomCss';
import DialogBox from './DialogBox/DialogBox';

const PackagesListMain = () => {
  LocalBaseURL()
...
//  to fetch data on first time rendering 
  ...
  useEffect(() => {
    ...
    async function InvokedRendered() {
      const { data } = await GetallPackagesData(localKeyCheck)
    ...
    <div>
      {
        isLoading ?
          ...
          <DataTable
            ...
            actions={
              <>
              <button className="btn btn-sm btn-primary" onClick={(row) => handlePackagesList()}><i className="fa fa-plus-circle mr-2"></i>Add Service Price</button>
 ...
 {PopUpPostFormOpen && <DialogBox PopUpPackagesList={handlePackagesList} PopUpPostFormOpen={PopUpPostFormOpen} />}
      
    </div>
  )
}
export default PackagesListMain
