...
import { GetAllAreasData } from './API';
...
//  to fetch data on first time rendering 
  ...
  useEffect(() => {
    ...
    async function InvokedRendered() {
      const { data } = await GetAllAreasData(localKeyCheck)
    ...
    <div>
      {
        isLoading ?
          ...
          <DataTable
            ...
            actions={
              <>
              <button className="btn btn-sm btn-primary" onClick={(row) => PopUpAreasBTN()}><i className="fa fa-plus-circle mr-2"></i>Add Service Price</button>
 ...
 {PopUpPostFormOpen && <DialogBox PopUpAreasBTN={PopUpAreasBTN} PopUpPostFormOpen={PopUpPostFormOpen} />}
      
    </div>
  )
}
export default AeasListMains
