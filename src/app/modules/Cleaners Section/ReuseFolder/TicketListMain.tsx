...
import { AdminListDataApi, cleanerDataApi, CustomerDataApi, localCheckAPI, payloadsInterfaces, SuperVisorDataApi, CleanerAttendance, TicektSourcesDataApi, TicektSubCategoryDataApi, ticketCategoryDataApi, inputEventChanger } from '../../../consts/Inerfaces4az/CleanerAttendance';
const ChangeNamethisfile = () => {
  LocalBaseURL()
  const localKeyCheck = JSON.parse(localStorage.getItem("API") || "0")
  const [pending, setPending] = React.useState(true);
  const [PopUpPostFormOpen, setPopUpPostFormOpen] = useState(false)
  const [filterData, setfilterData] = useState<CleanerAttendance[]>([])
  const [TicketsData, setTicketsData] = React.useState<CleanerAttendance[]>([])
...
export default ChangeNamethisfile
